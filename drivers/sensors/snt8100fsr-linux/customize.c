	        ...
	        if (gs_found == 0){
				...
				if(g_ASUS_hwID >= HW_REV_ER){
					if(p->tap > 0){
						if((p->tap & 0x10)>0){
							...
							input_event_report(gesture_id, count, 1, tr[i].bar_id, tr[i].force_lvl, frame, tr[i].center);
						...
						if((p->tap & 0x1)>0){
							gesture_id = GRIP_TAP_L1;
							input_event_report(gesture_id, count, 0, tr[i].bar_id, tr[i].force_lvl, frame, tr[i].center);
				...
				if(gesture_id == GRIP_SWIPE_L){
					if(p->swipe0_velocity > 0) //bottom to top
						input_event_report(gesture_id, 0, 0, 0 ,p->swipe0_velocity, 0, 0);
					else//top to bottom
						input_event_report(gesture_id, 0, 1, 0 ,p->swipe0_velocity, 0, 0);
				}else if(gesture_id == GRIP_SWIPE_R){
					if(p->swipe1_velocity > 0) //bottom to top
						input_event_report(gesture_id, 0, 0, 0 ,p->swipe1_velocity, 0, 0);
					else//top to bottom
						input_event_report(gesture_id, 0, 1, 0 ,p->swipe1_velocity, 0, 0);
				}else{
				}
				if(gesture_id!=0)
				...
				if (p->escape0 == GS_RPT_TYPE_SLIDE) {
					...
					*/
					...
					if(p_sq->squeeze[0] > 0){
						...
						input_event_report(gesture_id, count,  sq2_gesture, tr[i].bar_id, tr[i].force_lvl, frame, tr[i].center);						
					}
					if(p_sq->squeeze[1] > 0){
						gesture_id = 0;
						sq2_gesture = 1;
...
void process_trigger(int trig_id, int gpio_value)
{
	...
	if(g_ASUS_hwID >= HW_REV_ER){
		if(gpio_value == 1){
			...
			input_event_report(trig_id+1, 0, 0, 0 ,0, 0, 0);
		}else{
			PRINT_INFO("Gesture: Tap%d, finger %s!", trig_id, "up");
			input_event_report(trig_id+1, 0, 1, 0 ,0, 0, 0);
		}
	}else{
		/* Do nothing */
	}
