...
/*sm8250*/
#define ZF7_DxO_REMOSAIC_CALI_START 0x821
#define ZF7_DxO_REMOSAIC_CALI_END 0x13F2
...
bool check_eeprom_module_in_table(struct cam_eeprom_ctrl_t * e_ctrl,uint8_t *eeprom_module_group_index) {
	...
	for(i=0;i<EEPROM_GROUP_SIZE;i++) {
		if(e_ctrl->cal_data.mapdata[g_eeprom_group[i].moduleid_offset] == g_eeprom_group[i].eeprom_module_Id) {
			pr_info("module_Id(0x%x) in table[%u]",e_ctrl->cal_data.mapdata[g_eeprom_group[i].moduleid_offset],i);
			*eeprom_module_group_index = i;
			return true;
		}
	}
	pr_err("No match module_id(0x%x) in table, please add this module id you need in table g_eeprom_group !!!\n",e_ctrl->cal_data.mapdata[g_eeprom_group[i].moduleid_offset]);
	return false;
}
...
*/
	...
	if((camera_id == 0 || camera_id == 2) && !g_otp_data_override_done[camera_id])
	{
		...
		}else if ( g_eeprom_info[0].mapdata[7157] != 0x68 ){ //Check 13M (camera_id == 2)Module ID in EEPROM 0
			pr_err("g_eeprom_info[0].mapdata[7157] = 0x%02X \n", g_eeprom_info[0].mapdata[7157]);
		}else{
...
void cam_eeprom_read_from_otp(struct cam_eeprom_ctrl_t * e_ctrl, int camera_id) {
	...
	for (i=2; i>=0; i--) {
		if(g_otp_data_banks[camera_id][i*OTP_DATA_LEN_BYTE+OTP_MODULEID_OFFSET] == 0x00 || g_otp_data_banks[camera_id][i*OTP_DATA_LEN_BYTE+OTP_MODULEID_OFFSET] == 0xFF) {
...
static struct device_attribute dev_attr_camera_resolutions[MAX_CAMERA_ID + 1] ={
	__ATTR(SYSFS_ATTR_CAMERA0, S_IRUGO | S_IWUSR | S_IWGRP, resolutions_read, NULL),
	__ATTR(SYSFS_ATTR_CAMERA1, S_IRUGO | S_IWUSR | S_IWGRP, resolutions_read, NULL),
	__ATTR(SYSFS_ATTR_CAMERA2, S_IRUGO | S_IWUSR | S_IWGRP, resolutions_read, NULL),
	__ATTR(SYSFS_ATTR_CAMERA3, S_IRUGO | S_IWUSR | S_IWGRP, resolutions_read, NULL),
	__ATTR(SYSFS_ATTR_CAMERA4, S_IRUGO | S_IWUSR | S_IWGRP, resolutions_read, NULL),
};
...
{
	__ATTR(SYSFS_ATTR_CAMERA0, S_IRUGO | S_IWUSR | S_IWGRP, status_read, NULL),
	__ATTR(SYSFS_ATTR_CAMERA1, S_IRUGO | S_IWUSR | S_IWGRP, status_read, NULL),
	__ATTR(SYSFS_ATTR_CAMERA2, S_IRUGO | S_IWUSR | S_IWGRP, status_read, NULL),
	__ATTR(SYSFS_ATTR_CAMERA3, S_IRUGO | S_IWUSR | S_IWGRP, status_read, NULL),
	__ATTR(SYSFS_ATTR_CAMERA4, S_IRUGO | S_IWUSR | S_IWGRP, status_read, NULL),
};
...
static void copy_to_dit_eeprom(struct cam_eeprom_ctrl_t *e_ctrl, uint8_t moduleGroupIndex)
{
	...
	pr_info("eeprom copy to dit eeprom e_ctrl->cal_data.num_data = %d",e_ctrl->cal_data.num_data);
...
static int cover_dit_cal_data(struct cam_eeprom_ctrl_t *e_ctrl, uint8_t moduleGroupIndex)
{
	...
	num = read_file_into_buffer(load_file_name, e_ctrl->cal_data.mapdata, EEPROM_SIZE);
	pr_info("[DIT_EEPROM] module_id 0x%x Start, num=%d", module_id, num);
	...
	{
		pr_info("[DIT_EEPROM] read vendor/factory file fail, read %d bytes from golden file %s module_id = 0x%x", num,dit_dut_factory_file_dir, module_id);
	}else{
...
//ASUS_BSP +++Jason_yeh suporrt sensor version features
			if(module_id==0x6a || module_id==0x6b || module_id==0x71)
...
void eeprom_dump_create(struct cam_eeprom_ctrl_t * e_ctrl,uint8_t moduleGroupIndex)
{
	...
	if(moduleGroupIndex == EEPROM_GROUP_SIZE) {
		pr_err("No match module_id(%u) in table, return directly\n", e_ctrl->cal_data.mapdata[g_eeprom_group[moduleGroupIndex].moduleid_offset]);
		return;
	}
